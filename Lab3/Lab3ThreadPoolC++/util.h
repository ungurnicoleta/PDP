...
#ifndef LAB3THREADPOOLC___UTIL_H
#define LAB3THREADPOOLC___UTIL_H
...
public:
    ...
    static void matrix_sum(int starting_line, int finish_line, vector<vector<int>> matrix1,
                           vector<vector<int>> matrix2,
                           const shared_ptr<vector<vector<int>>> &result) {
        for (int i = starting_line; i < finish_line; i++)
            for (int j = 0; j < matrix1.size(); j++)
                (*result)[i][j] = (matrix1[i][j] + matrix2[i][j]);
    }

    static void matrix_product(int starting_line, int finish_line, vector<vector<int>> matrix1,
                               vector<vector<int>> matrix2,
                               const shared_ptr<vector<vector<int>>> &result) {
        for (int i = starting_line; i < finish_line; i++)
            for (int j = 0; j < matrix1.size(); j++)
...
#endif //LAB3THREADPOOLC___UTIL_H
